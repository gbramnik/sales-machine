...
- [x] **Task 1: Create api_credentials table in Supabase** (AC: 1, 7)
  - [x] Create migration file: `supabase/migrations/YYYYMMDD_create_api_credentials.sql`
...
- [x] **Task 2: Create SettingsService** (AC: 1, 2, 3, 4)
  ...
  - [x] Implement `verifyApiCredential(userId, serviceName)`:
    ...
    - **Service-specific verification:**
      - `openai` (Claude API): Test by calling `GET https://api.anthropic.com/v1/messages` with `api_key` (or equivalent endpoint)
      - `unipil`: Test by calling `GET {{ UNIPIL_API_URL }}/api/v1/health` with `api_key` (or equivalent)
      - `n8n_*` webhooks: Test by sending `POST $webhook_url` with `{ test: true }` payload
      - `smtp_*`: Test by connecting to SMTP server with credentials (use `nodemailer` test connection)
      - `cal_com` / `calendly`: Test by calling OAuth validation endpoint
...
- apps/api/migrations/20250111_create_api_credentials.sql
...
- apps/api/src/services/SettingsService.ts (fixed methods to use JSONB fields)
