...
- [ ] **Task 2: Initialize GitHub repository with protection** (AC: 2)
  - [ ] Initialize Git repository and create .gitignore file (node_modules, .env, dist/, coverage/)
  - [ ] Create README.md with project overview and setup quickstart
  - [ ] Push initial commit to GitHub repository
  - [ ] Configure branch protection rules for main branch (require PR, passing CI checks)

- [ ] **Task 3: Create Supabase project and configure Auth** (AC: 3)
  - [ ] Create Supabase project in eu-west-1 region via Supabase dashboard
  - [ ] Configure OAuth2 providers: Google OAuth (client ID/secret) and LinkedIn OAuth (client ID/secret) in Supabase Auth settings
  - [ ] Test OAuth flow locally using Supabase CLI `supabase start`
  - [ ] Generate Supabase connection credentials (Database URL, Anon Key, Service Role Key)
  - [ ] Document Supabase project URL and keys in .env.example file

- [ ] **Task 4: Create Upstash Redis instance** (AC: 4)
  - [ ] Create Upstash Redis database (free tier, eu-west-1 region) via Upstash dashboard
  - [ ] Copy REST API URL and token from Upstash dashboard
  - [ ] Add REDIS_URL environment variable to .env.example
  - [ ] Test Redis connection using curl or Upstash REST API client

- [ ] **Task 5: Setup N8N Cloud workspace** (AC: 5)
  - [ ] Create N8N Cloud account and workspace
  - [ ] Generate N8N API key from workspace settings
  - [ ] Create test webhook in N8N to verify webhook URL format
  - [ ] Document N8N webhook base URL pattern in .env.example (N8N_WEBHOOK_URL)
  - [ ] Create /workflows/README.md documenting webhook endpoints and deployment process

- [ ] **Task 6: Create Railway project for API Gateway** (AC: 6)
  - [ ] Create Railway account and new project
  - [ ] Configure Railway project for Europe deployment region (Frankfurt)
  - [ ] Connect Railway project to GitHub repository for auto-deploy
  - [ ] Add environment variables to Railway: DATABASE_URL (Supabase), REDIS_URL (Upstash), N8N_WEBHOOK_URL, N8N_API_KEY
  - [ ] Configure Railway build settings: Build command, start command, root directory
  - [ ] Deploy initial "Hello World" API to verify Railway deployment works

- [ ] **Task 7: Configure GitHub Actions CI/CD pipeline** (AC: 7)
  - [ ] Create .github/workflows/ci.yaml for linting, type-checking, and tests
  - [ ] Add npm scripts to root package.json: lint, type-check, test, build
  - [ ] Configure CI to run on pull_request and push events
  - [ ] Test CI pipeline by creating test PR
  - [ ] Add CI status badge to README.md

- [ ] **Task 8: Create local development setup documentation** (AC: 8)
  - [ ] Create /docs/dev-setup.md with prerequisites (Node 20 LTS, npm 10+, Supabase CLI)
  - [ ] Document initial setup steps: clone repo, npm install, supabase start, generate types
  - [ ] Document development commands: npm run dev, npm run dev:web, npm run dev:api
  - [ ] Include troubleshooting section for common setup issues
  - [ ] Test documentation by following steps on fresh machine or clean clone

- [ ] **Task 9: Create and test N8N workflow deployment script** (AC: 9)
  - [ ] Create scripts/deploy-workflows.sh script using N8N API for workflow import
  - [ ] Add error handling and validation (check N8N_API_KEY exists, handle upload failures)
  - [ ] Test script by deploying sample workflow JSON to N8N Cloud
  - [ ] Update /workflows/README.md with deployment instructions
  - [ ] Add deploy:workflows npm script to root package.json
  - [ ] Create .github/workflows/deploy-workflows.yaml with manual approval gate
  - [ ] Test deployment workflow in GitHub Actions
